    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // --- NEW: Setup Flow Controls ---
                localStorage.setItem('artron_setup_flow_step', (data.user.setupStep || 0).toString());

                // Redirect based on setup completeness
                if ((data.user.setupStep || 0) < 5) {
                    router.push('/settings'); // Force setup wizard
                } else {
